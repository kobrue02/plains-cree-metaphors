...
VALID_GOLD_PARQUET = "data/figurative/eval_validation_gold.parquet"
VALID_FULL_PARQUET = "data/figurative/eval_validation_full.parquet"
...
def load_rows(parquet_path: str, models: list[str]) -> pd.DataFrame | None:
    ...
    skip message) if the file or any required, non-errored row is missing."""
    if not os.path.exists(parquet_path):
        return None
    df = pd.read_parquet(parquet_path).set_index("model")
...
def make_ablation_heatmap() -> None:
    ...
    df = load_rows(VALID_GOLD_PARQUET, models)
    if df is None:
        skip(name, f"{VALID_GOLD_PARQUET} not ready — run jobs/evaluate/eval_validation_set.sh")
        return
...
def make_alpha_sweep() -> None:
    ...
    gold = load_rows(VALID_GOLD_PARQUET, models)
    full = load_rows(VALID_FULL_PARQUET, models)
    if gold is None or full is None:
        missing = VALID_GOLD_PARQUET if gold is None else VALID_FULL_PARQUET
...
if len(written_figures) < 4:
    print(f"\n{4 - len(written_figures)} figure(s) skipped — re-run once the corresponding eval job finishes.")
