#!/bin/bash
#SBATCH --job-name=CLKD_XLM_Frozen12
...
# CLKD with the first 12 XLM layers frozen.
#
...
# Layers 0-11 frozen → cross-lingual alignment from TLM preserved.
# Layers 12-15 + classification head trained on Cree via KL distillation.
...
# 4. Run CLKD
echo "Starting CLKD (XLM, frozen layers 0-11)..."
...
python3 main.py \
    ...
    --distill-freeze-layers 12 \
    --distill-output data/figurative/clkd_frozen12 \
    --epochs 10 \
    --batch-size 16 \
...
if [ $? -eq 0 ]; then
    ...
    echo "CLKD (frozen12) failed with exit code $?."
    exit 1
fi
